...
* ----------------------
*  ver 2.0 : 30 Apr 2018
*  - add checkin, checkout, queryrequest function
*  - add version and root in the instance
*  ver 1.2 : 22 Apr 2018
...
namespace Repository
{
  ...
  public:
    Server(MsgPassingCommunication::EndPoint ep, const std::string& name);
		ServerProc SendFile();
		ServerProc GetFile();
		ServerProc CheckInFile();
		ServerProc CloseCheckIn();
		ServerProc CheckOutFile();
		ServerProc QueryRequest();
		ServerProc Display();
		std::vector<std::string> GetChildren(std::string Key);
		bool contains(const std::string& key) { return db_.contains(key); };
		bool CopyFileToRepo(const std::string src, const std::string dst);
		static std::string KeyToPath(std::string key);
    void start();
    void stop();
    ...
    void initializeDb();
		...
		Version& versions() { return versions_; };
		Version versions() const { return versions_; };
  ...
  private:
	...
	//----< translate key string to file path string >----------------------------
	std::string Server::KeyToPath(std::string key)
	{
		int pos = key.find_first_of(":");
		std::string s = key;
		if (pos != std::string::npos && static_cast<unsigned int>(pos) < key.length() - 1)
		{
			s.replace(pos, 2, "_");
		}
		return s;
	...
	//----< get children of key >----------------------------
	std::vector<std::string> Server::GetChildren(std::string Key)
	{
		...
		while (!qu.empty())
		{
			s = qu.front();
			qu.pop();
			auto temp = visited.find(s);
			if (temp == visited.end())
			{
				visited.insert(s);
				if (!db_[s].children().empty())
				{
					for (auto child : db_[s].children())
						qu.push(child);
				}
			}
		}
		ch.insert(ch.end(), visited.begin(), visited.end());
		return ch;
	}
	//----< file copy >----------------------------
	bool Server::CopyFileToRepo(const std::string src, const std::string dst)
	{
		if (!FileSystem::File::exists(src))
			return true;
		if (!FileSystem::File::copy(src, dst, false))
			return false;
  ...
  inline Server::Server(MsgPassingCommunication::EndPoint ep, const std::string& name)
		: comm_(ep, name), versions_({}), root_("../Storage/") {
    initializeDb();
  }
